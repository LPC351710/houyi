 ...
 */
public class InfiniteViewPager extends FixedViewPager {
...
//                log("onPageScrolled:" + position + "->" + positionOffset + "-" +
// positionOffsetPixels);
                if (mOnPageChangeListener != null) {
                    mOnPageChangeListener.onPageScrolled(FakePositionHelper.getFakeFromReal
                                    (InfiniteViewPager.this, position), positionOffset,
                            positionOffsetPixels);
                }
            }
...
//                log("onPageSelected:" + position);
                if (position < FakePositionHelper.getStartPosition(InfiniteViewPager.this) ||
                        position > FakePositionHelper.getEndPosition(InfiniteViewPager.this)) {
                    log("position:" + position + "->" + FakePositionHelper.getRealPositon
                            (InfiniteViewPager.this, position) + "-return");
                    ...
                    log("position:" + position + "->" + FakePositionHelper.getRealPositon
                            (InfiniteViewPager.this, position));
                }
                if (mOnPageChangeListener != null) {
                    mOnPageChangeListener.onPageSelected(FakePositionHelper.getFakeFromReal
                            (InfiniteViewPager.this, position));
                }
            }
...
//                log("onPageScrollStateChanged->" + sta);
            ...
            public void dispatchMessage(Message msg) {
                switch (msg.what) {
                    case MSG_AUTO_SCROLL:
                        ...
                        break;
                    case MSG_SET_PAGE:
                        setFakeCurrentItem(FakePositionHelper.getRealPositon(InfiniteViewPager
                                .this, msg.arg1), false);
                        break;
                }
            }
    ...
    private void setItemToNext() {
        ...
        int totalCount = isInfinitePagerAdapter() ? FakePositionHelper.getRealAdapterSize(this) :
                adapter.getCount();
        if (totalCount <= 1)
            return;
...
//        getParent().requestDisallowInterceptTouchEvent(true);
    ...
    public static class FakePositionHelper {
        ...
        public static int getRealFromFake(InfiniteViewPager viewPager, int fake) {
            ...
            int real = fake + (currentReal - currentReal % realAdapterSize);//set to the target
            // level
            return real;
        }
        ...
        public static int getRealAdapterSize(InfiniteViewPager viewPager) {
            return viewPager.isInfinitePagerAdapter() ? viewPager.getAdapterSize() / MULTIPLIER :
                    viewPager.getAdapterSize();
        }
        ...
        public static int getAdapterSize(ViewPager viewPager) {
            ...
            if (adapter instanceof InfinitePagerAdapter) {
                InfinitePagerAdapter infinitePagerAdapter = (InfinitePagerAdapter) viewPager
                        .getAdapter();
